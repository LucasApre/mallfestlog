...
<div class="container-fluid">
    <div class="row">
        <div class="col-md-6">
            <label for="" class="text-muted">Código de referência</label>
            <div class="ml-3"><b><?= isset($ref_code) ? $ref_code : "N/A" ?></b></div>
        </div>
        <div class="col-md-6">
            <label for="" class="text-muted">Data do pedido</label>
        ...
        <div class="col-md-6">
            ...
            <div class="ml-3">
                <?php if(isset($status)): ?>
                    ...
                    <?php elseif($status == 1): ?>
                        <span class="badge badge-primary px-3 rounded-pill">Em separação</span>
                    <?php elseif($status == 2): ?>
                        <span class="badge badge-success px-3 rounded-pill">Em Transferencia</span>
                    <?php elseif($status == 3): ?>
                        <span class="badge badge-warning px-3 rounded-pill">Em Rota de Entrega</span>
                    <?php elseif($status == 4): ?>
                        <span class="badge badge-default bg-gradient-teal px-3 rounded-pill">Entregue</span>
                    <?php else: ?>
                        <span class="badge badge-danger px-3 rounded-pill">Entregue</span>
                    <?php endif; ?>
                <?php else: ?>
                    N/A
                <?php endif; ?>
...
<script>
    $('#btn-cancel').click(function(){
        _conf("Tem certeza que deseja cancelar este pedido?","cancel_order",[])
    })
