...
<div class="content py-5 mt-3">
    <div class="container">
        <h3><b>Meu carrinho de compras</b></h3>
        <hr>
        ...
        <div class="text-right">
            <button class="btn btn-flat btn-sm btn-dark" type="button" id="checkout">Confirmar</button>
        </div>
    </div>
</div>
<script>
    ...
    $(function(){
        ...
        $('.btn-remove').click(function(){
            _conf("Tem certeza que deseja remover este produto da lista do carrinho?","remove_from_cart",[$(this).attr('data-id')])
        })
        $('#checkout').click(function(){
            if($('#cart-list .cart-item').length > 0){
                location.href="./?p=place_order"
            }else{
                alert_toast('O carrinho de compras está vazio','error')
            }
        })
    })
